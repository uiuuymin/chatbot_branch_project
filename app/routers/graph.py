...
def session_graph(
    ...
    - **nodes**: 브랜치 하나하나가 node. label(브랜치 이름), status, message_count 포함
    - **edges**: parent → child 분기 관계. fork_from_message_id로 어느 메시지에서 갈라졌는지 표시
    ...
    return graph_service.build_branch_graph(branches, message_counts, include_inactive)
